...
NUTRITION_DATABASE = {
    ...
    "broccoli florets": {"calories": 34, "protein": 2.8, "carbs": 7, "fat": 0.4, "fiber": 2.4, "sodium": 64, "sugar": 1.4},
    ...
    "berries": {"calories": 40, "protein": 0.7, "carbs": 10, "fat": 0.3, "fiber": 2, "sodium": 3, "sugar": 6},
...
def validate_nutrition_data(nutrition_dict: dict) -> dict:
    ...
    # If protein seems too high relative to calories
    if corrected.get("protein", 0) > 0:
        max_protein_calories = corrected["protein"] * 4  # 4 cal per gram
        if max_protein_calories > corrected.get("calories", 0) * 0.8:
            # Protein is reasonable if it fits within calorie budget
            pass
    
    # Ensure carbs + protein + fat doesn't massively exceed calories
    # (allowing for rounding: 1g protein = 4 cal, 1g carbs = 4 cal, 1g fat = 9 cal)
    calculated_cals = (
    ...
    # If calculated cals are way off, adjust the largest value
    reported_cals = corrected.get("calories", calculated_cals)
    if calculated_cals > 0 and abs(calculated_cals - reported_cals) > reported_cals * 0.2:
        # There's a discrepancy - this is expected for estimates
        # Keep reported value but note it's approximate
        pass
